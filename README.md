...
- Use the same scikit-learn version that was used for training (1.3.2). This project pins it in `requirements.txt`.
...
# install deps (includes scikit-learn==1.3.2)
...
Troubleshooting
- InconsistentVersionWarning when loading the model: This appears if scikit-learn in your environment doesn’t match the version used during training. Installing `scikit-learn==1.3.2` (as pinned) resolves it.
- monotonic_cst attribute error: Caused by version mismatch in scikit-learn pickles. The app includes a compatibility shim, but the recommended fix is to use `scikit-learn==1.3.2` or re-save the model with your current scikit-learn version.
# Future Enhancements
